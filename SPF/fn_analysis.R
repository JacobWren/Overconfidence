...
fn_analysis <- function(df_indvl, df_agg, df_collapsed_bin_all_vars_indvl) {
  # Individual level:
  ...
  # Individual level
  smoothing_param <- 1.475 # Used for smoothing scatterplot via a spline.
  data_idvl_smoothed <-
  ...
  names(df_collapsed_bin_all_vars_indvl)[names(df_collapsed_bin_all_vars_indvl) == "dataSet"] <- "forecast_var"
  # Combining groups ==> that is, taking averages over forecasted variables/timeToResolution.
  df_collapsed_bin_all_vars_indvl <- df_collapsed_bin_all_vars_indvl %>%
    # Create and modify 'distance_from_resolution_trucate'.
    mutate(
      distance_from_resolution_trucate = if_else(distance_from_resolution > 2, 1.875, distance_from_resolution)
    ) %>%
    group_by(forecast_var, distance_from_resolution_trucate) %>%
    mutate(grpd_forecast_var_distance = cur_group_id()) %>%
    ungroup()
  
  data_collapsed <- df_collapsed_bin_all_vars_indvl %>%
  ...
  # Regressions
  var_regs_int_avg <-
    fn_variance_regs(data_collapsed, simple_reg = TRUE)
  var_regs_no_int_avg <-
  ...
  data_fully_collapsed <- df_collapsed_bin_all_vars_indvl %>%
    summarise(
      error = mean(error, na.rm = TRUE),
      disagreement_EV_minus_person_i = mean(disagreement_EV_minus_person_i, na.rm = TRUE)
    )
  ...
  stargazer_table_disagg <- stargazer(
    ...
    add.lines = list(c(
      ...
      sprintf("%.4f", summary(var_regs_int_avg)$coefficients["disagreement_EV_minus_person_i", "Pr(>|t|)"]),
      sprintf("%.4f", summary(var_regs_no_int_avg)$coefficients["disagreement_EV_minus_person_i", "Pr(>|t|)"]),
      ""
    )),
